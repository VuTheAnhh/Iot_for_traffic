...
def khoang_cach_ngan_nhat(ds_khoang_cach, ds_dinh_cay_ngan_nhat):
    ...
    for dinh in range(len(ds_khoang_cach)):
        if ds_dinh_cay_ngan_nhat[dinh]  == False \
                    and ds_khoang_cach[dinh] < nho_nhat:
...
def dijkstra(do_thi, dinh_nguon, dinh_dich):
    ...
    for i in range(so_luong_dinh):
        ...
        if x == dinh_dich:
            ...
            thong_bao = f"Tu dinh {dinh_nguon} den dinh {dinh_dich} : " \
                    + duong_di + " : " + str(ds_khoang_cach[dinh_dich])
            print(thong_bao)
            return
        else:
            for y in range(so_luong_dinh):
                if ds_dinh_cay_ngan_nhat[y] == False \
                        and do_thi[x][y] > 0 \
                            and ds_khoang_cach[y] > ds_khoang_cach[x] + do_thi[x][y]:
...
def main():
    ...
    dinh_dich = int(input("Nhap vao dinh dich: "))
